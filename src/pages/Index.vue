  ...
  <main class="w-screen">
    <section class="flex items-center relative overflow-hidden h-[75vh] !m-0">
      <div class="flex flex-col mx-24 my-36">
        <h1 class="split text-white font-bold text-4xl lg:text-8xl my-2 text-center lg:text-left"><span>It </span><span>all </span><span>starts </span><br><span>with&nbsp;</span><span>data.</span></h1>
        ...
        <button class="bg-teal-900 hover:bg-teal-500 hover:scale-110 transition-all rounded-3xl my-2 py-3 px-5 text-white hover:text-teal-900 font-bold lg:ml-0 mx-auto text-sm lg:text-lg"><router-link to="/">Find Out How</router-link></button>
      </div>
    ...
    <section class="appear mx-2 lg:mx-24 rounded-3xl shadow-lg bg-white py-12 lg:px-36 px-4 flex lg:justify-between items-center justify-center lg:flex-row flex-col lg:gap-0 gap-5">
      ...
      <div class="flex content-center flex-col items-center h-full">
        <div class="lg:m-5 flex items-center flex-wrap justify-center">
          <img class="object-contain w-12 lg:w-36 h-auto my-2 mx-5" v-for="client in clients.slice(0,3)" :src="`/logo/${client.replace(/ /g,'_')}.png`" :alt="client"/>
        </div>
        <div class="lg:m-5 flex items-center flex-wrap justify-center">
          <img class="object-contain w-12 lg:w-36 h-auto my-2 mx-5" v-for="client in clients.slice(-4)" :src="`/logo/${client.replace(/ /g, '_')}.png`" :alt="client"/>
        </div>
      </div>
    </section>
    ...
    <section>
      <h2 class="split font-bold text-center lg:text-left"><span>Data </span><span class="text-teal-900">First,</span><br><span>Innovation</span><span> Follows.</span></h2>
      ...
      :class="[index === 1 ? 'lg:flex-row-reverse' : 'lg:flex-row']" v-for="(solution, index) in solutions">
        <img v-if="width > 1024" class="object-cover lg:h-full h-1/3 lg:w-1/3 w-full basis-1/3 flex-grow-0 flex-shrink-0" :src="`/solutions/${solution.image}`" :alt="solution.imageAlt"/>
        <div class="py-6 lg:py-12 px-10 from-transparent to-white relative z-10 flex flex-col justify-center gap-4 w-full lg:w-auto lg:max-w-[75%]"
        :class="[solution.dashboard ? 'to-30% lg:pl-24' : 'to-15% lg:pl-72', index >= 1 ? 'text-left items-start' : 'text-right items-end', index === 1 ? '-mr-52 bg-gradient-to-l !pl-10 !lg:pr-24' : 'lg:-ml-52 bg-gradient-to-r']">
          ...
          <p>{{ solution.desc }}</p>
          <button class="bg-white hover:bg-teal-900 transition-all hover:shadow-[0_0_50px_0_rgba(0,0,0,0.3)] hover:shadow-teal-900/80 duration-300 ring-teal-900 hover:ring-0 ring-1 rounded-3xl my-2 py-3 px-5 text-teal-900 hover:text-white font-bold text-sm lg:text-lg"
    ...
    <section>
      <div class="appear relative rounded-3xl bg-gray py-5 md:py-12 lg:px-36 px-12">
        ...
        <q class="font-bold leading-snug text-lg md:text-3xl lg:text-6xl text-teal-900 mt-3 w-11/12 block">It has been an eye-opening experience working with Datafirst Solutions, especially with their professional support and constant engagement in implementing different analytics projects.</q>
    ...
    <section>
      <div class="appear bg-gray rounded-t-3xl py-12 lg:px-36 px-12 h-[75vh] flex flex-col lg:flex-row justify-center items-center w-full">
        ...
        <button class="bg-teal-900 hover:scale-110 rounded-3xl lg:my-2 my-5 py-3 px-5 text-white font-bold lg:ml-auto transition-all duration-300 text-sm lg:text-lg hover:shadow-[0_0_50px_0_rgba(0,0,0,0.3)] hover:shadow-teal-900/80"><router-link to="/contact">Schedule a Demo</router-link></button>
      </div>
    </section>
  </main>
</template>
