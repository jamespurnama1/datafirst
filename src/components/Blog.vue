...
<template>
  <section class="appear !m-0 md:!mx-8 lg:!mx-24">
    <component :is="width > 768 || route.path === '/blog' ? 'div' : Swiper" :modules="modules" :observer="true" :space-between="50" :slides-per-view="1" :autoplay="{ delay: 5000 }" class="md:grid md:gap-5 lg:gap-12 md:grid-cols-2 lg:grid-cols-3 !p-8 md:!p-0">
      <component :is="width > 768 || route.path === '/blog' ? 'div' : SwiperSlide" v-for="(post, index) in result" class="h-[300px] lg:h-[512px] relative !flex flex-col justify-end shadow-lg bg-white overflow-hidden rounded-3xl lg:px-6 px-3 py-3 md:px-6 lg:py-12 my-5 first:mt-0">
